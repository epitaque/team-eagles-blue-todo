(function(){
    ...
    module.exports = function(app){
        ...
        router.post('/todo', function(req, res, next){
            ...
            if( userId && todo ){
                ...
                User.findById(userId, (err, user) => {
                    if(err) handleError(res, err.message, "Failed to get populated User.");

                    user.todos.push(todo);
                    user.save( err => {
                        if(err) handleError(res, err.message, "Failed to save User to DB.");
            ...
            if( userId && todo ){
                Todo.findOneAndUpdate({"_id": todo._id}, todo, {upsert: true}, (err, todo) => {
                    User.findOne({"username": userId})
                        .populate('todos')
                        .exec( (err, user) => {
                            if(err) handleError(res, err.message, "Failed to update Todo.");
                            else res.json(user.todos);
                        });
                });
            }
        });
        ...
        router.post('/deletetodo', function(req, res){
            ...
            if( userId && todo ){
                User.findById(userId, (err, user) => {
                    if(err) handleError(res, err.message, "Failed to find User in DB.");
                    ...
                    Todo.findById(todo._id)
                        .remove()
                        .exec( (err, data) => {
                            if(err) handleError(res, err.message, "Failed to delete Todo from database.");
                        });
                    // Save User data
                    user.save( err => {
                        if(err) handleError(res, err.message, "Failed to save new User info.");
                        else res.json(user);
                    });
                })
            }
