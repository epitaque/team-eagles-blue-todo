            ...
            if( userId && todo ){
                ...
                User.findById(userId, (err, user) => {
                    ...
                    user.save( err => {
                        ...
                        todo.save( err => {
                            if(err) {
                                handleError(res, err.message, "Failed to save Todo to DB.");
                                return;
                            }
            ...
            if( userId && todo ){
                User.findById(userId, (err, user) => {
                    ...
                    Todo.findById(todo._id)
                        ...
                        .exec( (err, data) => {
                            if(err) {
                                handleError(res, err.message, "Failed to delete Todo from database.");
                                return;
                            }
                        });
                    // Save User data
                    user.save( err => {
                        if(err) {
                            handleError(res, err.message, "Failed to save new User info.");
                            return;
                        }
                        else {
                            res.json(user);
